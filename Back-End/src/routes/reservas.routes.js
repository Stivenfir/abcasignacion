...
router.get("/disponibles/:fecha", authenticateToken, async (req, res) => {      
  ...
  try {      
    var Rta = await GetData(`ConsultaReservas=@P%3D2,@Fecha%3D'${fecha}'`);      
...
router.post("/", authenticateToken, async (req, res) => {      
  const { idPuestoTrabajo, fecha } = req.body;  // ✅ Solo estos del body  
  const idEmpleado = req.user.idEmpleado;  // ✅ Del token JWT  
  const usuario = req.user.username;      
  ...
  try {      
    ...
    // ✅ Corregido: usar 'fecha' en lugar de 'fechaReserva'  
...
router.put("/:idReserva/cancelar", authenticateToken, async (req, res) => {      
  ...
  try {      
    ...
    // ✅ Corregido: usar 'observacion' e 'idEmpleado' correctamente  
