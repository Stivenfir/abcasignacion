...
export function useAreas() {    
  ...
  const cargarDelimitaciones = async (idAreaPiso) => {    
    try {    
      ...
      // ✅ Normalizar: convertir idDelimitacion (minúscula) a IdDelimitacion (PascalCase)  
      const normalized = Array.isArray(data)   
        ? data.map(delim => ({  
            ...delim,  
            IdDelimitacion: delim.idDelimitacion || delim.IdDelimitacion  
          }))  
        : [];  
  ...
  const editarDelimitacion = async (idAreaPiso, idDelimitacion, rectangulo) => {    
    try {    
      ...
      // Recargar delimitaciones (ya normalizadas)  
