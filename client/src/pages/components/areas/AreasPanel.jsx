...
import GestionDelimitacionesModal from "./GestionDelimitacionesModal";   
...
export default function AreasPanel({  
  ...
  const [delimitacionAEditar, setDelimitacionAEditar] = useState(null);   
  const [modo, setModo] = useState(null); // 'crear' | 'editar' | 'agregar'  
  ...
  // Para editar delimitación específica  
  const handleAbrirPlanoEdicion = async (areaPiso, delimitacion) => {    
    setAreaSeleccionada(areas.find(a => a.IdArea === areaPiso.IdArea)?.IdArea);    
    setAreaPisoSeleccionada(areaPiso.IdAreaPiso);    
    setDelimitacionAEditar(delimitacion);    
    setModo('editar');    
    setPlanoVisible(true);    
  };  
  
  // Para agregar delimitación a área existente  
  const handleAbrirPlanoAgregar = async (areaPiso) => {    
    setAreaSeleccionada(areas.find(a => a.IdArea === areaPiso.IdArea)?.IdArea);    
    setAreaPisoSeleccionada(areaPiso.IdAreaPiso);    
    setDelimitacionAEditar(null);    
    setModo('agregar');    
    setPlanoVisible(true);    
  };    
  
  // Para crear área nueva (desde áreas disponibles)  
  ...
  const handleGestionarDelimitaciones = (areaPiso) => {  
    setModalGestion(areaPiso);  
  };  
    ...
    <div className="lg:col-span-2">  
      <div className="space-y-6">  
        ...
        <AreasAsignadas  
          ...
          onEditarDelimitacion={handleAbrirPlanoAgregar}  
          onEliminarArea={onEliminarArea}  
          onVerMapaCompleto={handleVerMapaCompleto}  
          onGestionarDelimitaciones={handleGestionarDelimitaciones}  
        />  
        ...
        <div className="mb-4">  
          ...
          <div className="mt-2 text-sm text-gray-600">  
            Mostrando {areasDisponiblesFiltradas.length} de {areas.length} áreas  
            disponibles  
          </div>  
        </div>  
      ...
      {planoVisible && (  
        <PlanoModal  
          ...
          areasPiso={areasPiso}
          delimitaciones={delimitaciones}  
          delimitacionAEditar={delimitacionAEditar}  
          onClose={() => {  
            setPlanoVisible(false);  
          ...
          onEditarDelimitacion={onEditarDelimitacion}  
        />  
      )}  
      ...
      {modalGestion && (  
        <GestionDelimitacionesModal  
          areaPiso={modalGestion}  
          area={areas.find((a) => a.IdArea === modalGestion.IdArea)}  
          delimitaciones={delimitaciones}  
          onEditarDelimitacion={handleAbrirPlanoEdicion}  
          onEliminarDelimitacion={onEliminarDelimitacion}  
