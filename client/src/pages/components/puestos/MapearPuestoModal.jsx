...
export default function MapearPuestoModal({
  ...
  useEffect(() => {
    if (puestoAMapear?.UbicacionX && puestoAMapear?.UbicacionY && canvasRef.current) {
    ...
    const cargarPuestosArea = async () => {
      try {
        ...
        setPuestosExistentes(
          data.filter(p => p.IdPuestoTrabajo !== puestoAMapear.IdPuestoTrabajo)
        );
      } catch (error) {
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between bg-gradient-to-r from-blue-50 to-indigo-50 rounded-t-2xl">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition ${mostrarGrilla
