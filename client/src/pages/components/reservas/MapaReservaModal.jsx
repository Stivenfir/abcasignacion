...
export default function MapaReservaModal({  
  ...
  const dibujarPuestoAsignado = () => {
    ...
    // 1) Coordenadas display (formato actual de mapeo)
    candidatos.push({ x: coords.x, y: coords.y, tipo: "display" });
    ...
    const delimitacionesValidas = (Array.isArray(delimitacionesArea) ? delimitacionesArea : []).map((d) => ({
      x: Number(d?.PosicionX),
      y: Number(d?.PosicionY),
      w: Number(d?.Ancho),
      h: Number(d?.Alto),
    })).filter((d) => [d.x, d.y, d.w, d.h].every(Number.isFinite));
