...
export default function MapaReservaModal({  
  ...
  useEffect(() => {
    const cargarDelimitacionesArea = async () => {
      ...
      try {
        ...
        const idAreaPisoReserva = reservaRender?.IdAreaPiso ?? reserva?.IdAreaPiso ?? areaAsignada?.IdAreaPiso;

        const areaEncontrada = listado.find((a) => {
          // Priorizar SIEMPRE IdAreaPiso exacto (más específico y evita tomar un área hermana)
        ...
        const delimitaciones = Array.isArray(dataDel) ? dataDel : [];

        // Defensa extra: si backend devuelve delimitaciones mezcladas, filtrar por IdAreaPiso cuando exista.
        const delimitacionesFiltradas = delimitaciones.filter((d) => {
          if (d?.IdAreaPiso == null || areaEncontrada?.IdAreaPiso == null) return true;
          return String(d.IdAreaPiso) === String(areaEncontrada.IdAreaPiso);
        });

        setDelimitacionesArea(delimitacionesFiltradas);
  ...
  const dibujarPuestoAsignado = () => {
    ...
    delimitacionesValidas.forEach((d) => {
